...
export async function getAllUsers() {
    ...
    const users = rows.map(row => new User(
      row.id, row.email, row.username, row.password, row.xp, row.role
    ));
    return users;
}
...
export async function getUser(id: number): Promise<User> {
    ...
    if (dbUsers.length > 0) {
        const user: User = new User(dbUsers[0].id, dbUsers[0].email, dbUsers[0].username, dbUsers[0].password, dbUsers[0].xp, dbUsers[0].role);
        return user;
    } else {
